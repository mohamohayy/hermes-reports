...
6. **Cloud free-tier API limits** — two distinct error modes:

   - **No API key / invalid key**: `/api/prompt`, `/api/view`, `/api/upload/*`,
     `/api/object_info` return **403**.
   - **Valid API key but no paid subscription**: returns **HTTP 429** with body
     `{"error": {"message": "Subscription required to queue workflows", "type": "PAYMENT_REQUIRED"}}`.
     Free tier can browse models but cannot run workflows via API.

   `health_check.py` and `check_deps.py` handle both cases gracefully and
