...
### Auth: ALWAYS Interactive (Pitfall)

**Authentication is always an interactive flow and CANNOT be automated via batch commands or terminal tool execution.** Even with `ANTHROPIC_API_KEY` set in the environment, Claude Code requires an initial OAuth handshake that involves:

1. Running `claude auth login --console` in an interactive terminal
2. Opening a browser URL (displayed by the CLI)
3. Logging in / authorizing on the Anthropic platform
4. Pasting the verification code back at the `Paste code here if prompted >` prompt

Without completing this interactive flow, `claude` will respond with `Not logged in · Please run /login` on every invocation, regardless of `ANTHROPIC_API_KEY` being set. The API key takes effect only AFTER the one-time OAuth registration is complete.

**For Hermes agents:** When a user asks to set up Claude Code auth, give them the instructions and have them do it manually in their own terminal. Do not try to run the auth command via terminal tool — it will time out waiting for input.
...
api.anthropic.com is blocked in mainland China. Claude Code cannot connect without a proxy. See `references/china-proxy-setup.md` for the full guide.

**Quick proxy check:** `claude -p "hello" --max-turns 1` — if it says "Unable to connect to Anthropic services" (ERR_BAD_REQUEST / ETIMEDOUT), you need a proxy.

**Windows native install + GFW:** `npm install -g @anthropic-ai/claude-code` works on Windows without issues. But `claude auth login --console` WILL hang or fail because api.anthropic.com is blocked in mainland China. The install itself succeeds — it's the auth step that's the blocker. The user needs a working proxy (机场 + Clash Verge, or similar) configured at the system level BEFORE running auth. Without a proxy, Claude Code is installed but unusable.

**Immediate workaround for WSL users:** if the Windows host runs Clash Verge or similar with system proxy enabled, WSL should inherit it automatically. Run `export HTTPS_PROXY=http://<Windows-IP>:<port>` if automatic detection fails.

**Full guide:** `references/china-proxy-setup.md`

## Pitfalls & Gotchas

1. **`ANTHROPIC_API_KEY` alone is NOT enough for first-time auth** — Claude Code requires `claude auth login --console` (interactive OAuth code paste) even when ANTHROPIC_API_KEY is already set. Once logged in, the key works for subsequent sessions.

2. **Interactive mode REQUIRES tmux**
2. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Using `pty=true` alone in Hermes terminal works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
3. **`--dangerously-skip-permissions` dialog defaults to "No, exit"** — you must send Down then Enter to accept. Print mode (`-p`) skips this entirely.
